 ...
 * Lists all generated proposal spaces with thumbnails, recommendations, and scene hashes.
 */
    ...
    <section className="space-y-6">
      ...
      <div className="rounded-3xl border border-white/10 bg-gradient-to-br from-violet-500/15 via-transparent to-cyan-500/15 p-6 shadow-[0_20px_80px_rgba(15,23,42,0.35)]">
        ...
        <p className="mt-2 max-w-3xl text-sm text-slate-300">
          Navigate through 360° spatial environments generated for governance proposals.
          Each environment maps proposal risk domains to spatial zones with multi-agent markers.
        </p>
        <div className="mt-5 grid gap-3 sm:grid-cols-3">
          <AtlasStat label="Total Environments" value={loading ? '...' : String(spaces.length)} />
          <AtlasStat label="Complete" value={loading ? '...' : String(spaces.filter(s => s.status === 'complete').length)} />
          <AtlasStat label="High Severity" value={loading ? '...' : String(spaces.filter(s => maxSeverity(s) === 'high').length)} />
        </div>
      </div>
      ...
      {!loading && filtered.length === 0 && !error && (
        <div className="rounded-2xl border border-white/10 bg-white/[0.03] p-8 text-center text-slate-400">
          No spatial environments generated yet. Go to the Governance page and click &ldquo;Generate Proposal Space&rdquo; on any proposal.
        </div>
      )}
